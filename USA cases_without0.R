...
codes <- read.csv(file = "states.csv", header = TRUE)
...
# for (i in 1:1) { 
    ...
    data_selected <- data.frame(
        positiveIncrease = case_by_date$positiveIncrease,
...
# ############################ File Write###########################################
write.xlsx(x = cbind(result,equal_variance), file = "04-30-11-20USA_coefficient_without0.xlsx")
